...
class WnReasonData:
  ...
  def __init__(self, path, negSampleSizeRatio, mapLoadPath=None, mapLoadNamePrefix=None):
    ...
    if mapLoadPath==None and mapLoadNamePrefix==None:
      self.e2id, self.uc2id, self.bc2id = self.getEntitiesConceptsMap(path+'/train.txt')
    else:
      self.loadEntityConceptMaps(mapLoadPath, mapLoadNamePrefix)
    #print('#Entities:', len(self.e2id), '#Relations:', len(self.bc2id))
    self.aboxUTripleLst, self.aboxBTripleLst = self.readAboxTripleList(path+'/train.txt')
    self.tboxUTripleLst, self.tboxBTripleLst = ([], []) 
    self.reldefTripleLst = []
    print('---------------------Start--------------------------')
    print('Training Data\n','#Triples:', len(self.aboxBTripleLst),'#Entities:', len(self.e2id), '#Relations:', len(self.bc2id))
    ...
    print('Negative Simpling\in','#Negative Triples: ', len(self.negAboxBTripleLst))
    ...
    self.allAboxUTripleLst, self.allAboxBTripleLst = self.readAllAboxTripleList(path+'/train.txt', path+'/valid.txt', path+'/test.txt')
    self.allUCMemberMap, self.allBCMemberMap = self.getAllClassMemberMap()
    self.testAboxUTripleLst, self.testAboxBTripleLst = self.readAboxTripleList(path+'/test.txt')
    self.testUCMemberMap, self.testBCMemberMap = self.getTestClassMemberMap()
    print('Testing Data\n', '#Triples: ', len(self.testAboxBTripleLst), '#Relations', len(self.testBCMemberMap.keys()))
    print('----------------------End----------------------------')
  ...
  def getTrainDataTensor(self, batchIndex, batchSize):
    ...
    return Variable(torch.LongTensor(aBHELst)), Variable(torch.LongTensor(aBTELst)), Variable(torch.LongTensor(aBCLst)),Variable(torch.LongTensor(aHeadLst)),Variable(torch.LongTensor(aTailLst)),Variable(torch.LongTensor(aRelationLst)), Variable(torch.LongTensor(nABHELst)), Variable(torch.LongTensor(nABTELst)), Variable(torch.LongTensor(nABCLst)), Variable(torch.LongTensor(nHeadLst)), Variable(torch.LongTensor(nTailLst)), Variable(torch.LongTensor(nRelationLst)), Variable(torch.LongTensor(uniqELst)), Variable(torch.LongTensor(uniqBCLst))
...
if __name__ == '__main__':
  ...
  dataPath = '/home/cpf/codes/e2r/umls/data/'
